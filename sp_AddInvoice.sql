...
--customerID
IF (@base_invoice_id IS NOT NULL AND @base_partial_invoice_id IS NOT NULL)
BEGIN
	SET @customer_id = (select customer_id from Invoice where invoice_id = @base_invoice_id
	and partial_invoice_id = @base_partial_invoice_id)
END
...
ELSE
	BEGIN
		IF (@base_invoice_id IS NULL)
		 BEGIN
			SET @newinvoice_id=(SELECT MAX(i.invoice_id)+1 FROM invoice i)		 
			SET @newpartial_invoice_id = 1			
		 END
		ELSE IF(@base_invoice_id IS NOT NULL AND @base_uuid IS NULL)
		BEGIN
			SET @newinvoice_id = @base_invoice_id		
			SET @newpartial_invoice_id=(SELECT MAX(i.partial_invoice_id)+1 FROM invoice i WHERE i.invoice_id=@base_invoice_id)
		END
...
@newpartial_invoice_id = @newpartial_invoice_id,
@notes = @notes,
@due_date = @due_date, 
...
and partial_invoice_id = @newpartial_invoice_id
...
BEGIN
  ...
  base_uuid)
  values
  (@newinvoice_id, @newpartial_invoice_id, @uuid, @duedatefee, @partialpayfee, @base_invoice_id, @old_due_date, @interestfee_sum, @base_invoice_id, @base_uuid)

  Update Invoice set notes=@notes, due_date = DATEADD(day, 14, Getdate()) where invoice_id = @newinvoice_id
  and uuid = @uuid and partial_invoice_id = @newpartial_invoice_id
  
  Update Invoice set due_date = @due_date where invoice_id = @base_invoice_id
  and uuid = @base_uuid and partial_invoice_id = @base_partial_invoice_id
  ...
  status = 2, invoicepaid = 1, payment_received_date = GETDATE()  
  where invoice_id = @base_invoice_id and uuid = @base_uuid  
  and partial_invoice_id = @base_partial_invoice_id
  and total_sum = @partial_amount_received
  --
...
--Partial Invoice
  ...
  BEGIN
   ...
   SELECT invoice_item_id, @newinvoice_id,@newpartial_invoice_id,@uuid,description, start_date,end_date,quantity,unit, @original_invoice_sum, vat_percent,vat,@original_invoice_sum,invoice_expense_id,invoice_allowance_id,partial_invoice_expense_id,partial_invoice_allowance_id FROM @invoice_items
  END
  ELSE
  BEGIN  
   INSERT INTO Invoice_items (invoice_item_id,invoice_id,partial_invoice_id,uuid,description,start_date,end_date,quantity,unit,quantity_price,vat_percent,vat,sum_tax_free,invoice_expense_id,invoice_allowance_id,partial_invoice_expense_id,partial_invoice_allowance_id)  
   SELECT invoice_item_id, @newinvoice_id,@newpartial_invoice_id,@uuid,description, start_date,end_date,quantity,unit,quantity_price,vat_percent,vat,sum_tax_free,invoice_expense_id,invoice_allowance_id,partial_invoice_expense_id,partial_invoice_allowance_id FROM @invoice_items  
 END   
  
   update invoice_expense set added_to_invoice =1, attach_option = 'add_to_invoice', invoice_id=@newinvoice_id,
   partial_invoice_id=@newpartial_invoice_id 
   where invoice_expense_id in (select invoice_expense_id from @invoice_items) 
   and partial_invoice_expense_id in (select partial_invoice_expense_id from @invoice_items)
   and uuid=@uuid  
   ...
   and partial_invoice_allowance_id in (select partial_invoice_allowance_id from @invoice_items)
   and uuid=@uuid 
  
   select @newinvoice_id as invoice_id, @customer_id as customersid, @uuid as invoice_uuid, @newpartial_invoice_id as partial_invoice_id	
